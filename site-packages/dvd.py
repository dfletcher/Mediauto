      ...
      if main_ts != None:
        ...
        if in_tsscan_mode:
          ...
          if ts:
            ...
            #for x in range(1, nangles+1):
            #  self.info['titlesets'][current_titleset]['titles'][x] = {}
          elif audio:
...
class DVD(process.MediautoProcessor):
  def __init__(self, info):
    ...
    if info['product'] != None: info['destfilename'] = normalize_dvd_title(info['product']) + '.iso'
    elif info['label'] != None:  info['destfilename'] = normalize_dvd_title(info['label']) + '.iso'
    else: info['destfilename'] = 'UNKNOWN_DISC.iso'
    info['destfile'] = os.path.join(DVD_DESTINATION, info['destfilename'])
